...
function OrderOneClick({ product }) {
  ...
  const order = async (e) => {
    ...
    try {
      ...
      setLoading(false);
      setPhoneValue("998");
      document.getElementById(`orderoneclick_${product.id}`).close();
  ...
  return (
    <>
      {status === "pedding" && (
        <button onClick={()=>{document.getElementById(`orderoneclick_${product.id}`).showModal();}} className="btn btn-sm w-full my-2">
          <span>Купить в 1 клик</span>
        </button>
      )}
      {status === "done" && (
        <button className="btn btn-sm w-full my-2 bg-maincolor text-white border-0 hover:bg-maincolor hover:text-white cursor-not-allowed">
          <span>Мы свяжемся с вами</span>
        </button>
      )}

      {/* Order Modal Start */}
      <dialog id={`orderoneclick_${product.id}`} className="modal">
        <Toaster />
        {/* <Toaster /> */}
          ...
          <div className="p-4">
            <div className="px-6 mb-4">
              ...
              <div className="flex gap-8 p-2">
                ...
                <div className="">
                  <p className="font-bold text-[14px] line-clamp-1">
                    {product.title}
                  </p>
                  ...
                  <p className="text-[12px]">
                    <span className="font-semibold">Цель: покупка</span>
                    {/* <span>
                      {product.width} x {product.length} x {product.height} см
                    </span> */}
                  </p>
                </div>
              </div>
            ...
            <form action="" className="px-6" onSubmit={order}>
